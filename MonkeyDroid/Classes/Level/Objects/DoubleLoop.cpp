...
DoubleLoop::DoubleLoop(b2World *world, CCNode *ccLayer, CCLayer *frontLayer, b2Vec2 *startPos):
...
void DoubleLoop::Update(float xpos,float ypos)
{
	...
	if(Rider::g_inLoop)
    {
        ...
        {
            AudioManager::PlayEffect(AUDIO_CHAR_LOOP2);
            Rider::g_BikeAudio->PushState(BikeAudio::LOOP2);
        }
        if(t >= 0.25f && prevT < 0.25f)
        {
                m_TopTexture->removeFromParentAndCleanup(false);
                m_BottomTexture->removeFromParentAndCleanup(false);
                m_frontLayer->addChild(m_TopTexture,1);
                m_frontLayer->addChild(m_BottomTexture,1);
        }
        ...
        else if(t >= 0.75f && prevT < 0.75f)
        {
            ...
            m_frontLayer->addChild(m_TopTexture2,0);
            m_frontLayer->addChild(m_BottomTexture2,0);
        }
...
/*        if(t >= 0.01f && prevT < 0.01f)
        ...
        else if(t >= 0.95f && prevT < 0.95f)
        {
            ...
            sprite.position = ccp(m_TopTexture2.position.x+175,m_TopTexture2.position.y-15);
            CCSequence *sequence = [CCSequence actions:[CCFadeIn actionWithDuration:0.1f],
                                    [CCDelayTime actionWithDuration:0.2f],
                                    [CCFadeOut actionWithDuration:0.1f],
                                    [CCCallFuncND actionWithTarget:sprite selector:@selector(removeFromParentAndCleanup:) data:(void*)YES],
...
void DoubleLoop::CreateSelf()
{
    ...
    CCSprite *temp = NULL;
    if(m_StartPos.y>1.0f)
        temp = CCSprite::createWithSpriteFrameName("ctm_UpperLoop_2c.png");
    else
        temp = CCSprite::createWithSpriteFrameName("ctm_BigLoop_New_2c.png");
    
    temp->setPosition(CCPointMake(PTM_RATIO*(m_StartPos.x+10.1f),PTM_RATIO*(m_StartPos.y+7.9f)));
    temp->setScale(1.0f);
    m_ccLayer->addChild(temp,2);
    m_BottomTexture3 = temp;
...
bool DoubleLoop::HandleBeginContact(b2Fixture *fixtureA,b2Fixture *fixtureB)
{
	...
	if(!Rider::g_inLoop && (m_ForwardTrigger == fixtureA || m_ForwardTrigger == fixtureB))
	{
        ...
        if(vel.x > 20.0f && goodAngle && goodY)
        {
            ...
            b2Vec2 points[12] =
            {
                ...
                b2Vec2(0.0f+m_StartPos.x+6.0f+20.0f,m_StartPos.y+11.0f),
            };
            
            if(m_Spline)
                delete m_Spline;
            m_Spline = new Spline(points, 12);
            ...
            Rider::g_ScoreManager->SetLoops(2);
